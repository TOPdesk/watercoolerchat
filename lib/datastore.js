...
const teamsroomsCollection = 'teamsrooms';
...
export const mongoDbDatastore = {
	...
	getAllTeamsRooms() {
		return this._db.collection(teamsroomsCollection).find({}).toArray();
	},
	removeTeamsRoom(teamsRoomId) {
		this._db.collection(teamsroomsCollection).deleteOne({_id: teamsRoomId});
	}
};
...
const inMemoryDatastore = {
	...
	_teamsRooms: [
		{
			_id: 1,
			url: 'https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZDk2MzJiOTktNTlhMi00ZTQ5LTk3NzQtNTk5ZmUwNWUzNmI4%40thread.v2/0'
		},
		{
			_id: 2,
			url: 'https://teams.microsoft.com/l/meetup-join/19%3ameeting_Y2FkNDc1YWYtYjhkOC00ZDdlLTk1MGMtNDhkM2VmN2Q2MTI4%40thread.v2/0'
		}
	],
	initialize() {
	...
	getAllTeamsRooms() {
		return Object.values(this._teamsRooms);
	},
	removeTeamsRoom(teamsRoomId) {
		this._teamsRooms = this._teamsRooms.filter(teamsRoom => teamsRoom._id !== teamsRoomId);
	}
};
