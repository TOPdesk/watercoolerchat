...
const QueueItem = ({
	...
	companyName, chatPartnerId, chatPartnerName, chatUrl, failedMatches
});

const addToQueue = async ctx => {
	const {userName, companyName} = ctx.request.body;
	const queueItem = QueueItem({userName, companyName}); // eslint-disable-line new-cap
