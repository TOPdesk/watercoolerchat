...
const notificationFilter = (companyName, mySubscriptionId) => {
	return subscription => {
		return subscription.companyName === companyName && subscription.subscriptionId !== mySubscriptionId;
	};
};
...
const sendNotificationsForQueueItem = queueItem => {
	...
	if (!lastNotificationSent[companyName] || lastNotificationSent[companyName] + MINIMUM_TIME_BETWEEN_NOTIFICATIONS_IN_MS < Date.now()) {
		...
		sendNotifications(notificationFilter(companyName, queueItem.subscriptionId), `Someone is looking for a chat at the watercooler at ${companyName}! Click here to go to the online watercooler.`);
	} else {
