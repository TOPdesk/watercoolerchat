...
let msTeamsLinkIndex = 0;
const msTeamsLinks = [
	'https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZDk2MzJiOTktNTlhMi00ZTQ5LTk3NzQtNTk5ZmUwNWUzNmI4%40thread.v2/0',
	'https://teams.microsoft.com/l/meetup-join/19%3ameeting_Y2FkNDc1YWYtYjhkOC00ZDdlLTk1MGMtNDhkM2VmN2Q2MTI4%40thread.v2/0'
];
...
const ChatGroup = ({
	...
	chatGroupId, companyName, chatUrl, lastMemberAddedDate, members, chatStarted
});
...
const generateMicrosoftTeamsUrl = () => {
	if (msTeamsLinkIndex >= msTeamsLinks.length) {
		msTeamsLinkIndex = 0;
	}

	const index = msTeamsLinkIndex;
	msTeamsLinkIndex = msTeamsLinkIndex + 1;
	return msTeamsLinks[index];
};

const createNewChatGroup = companyName => {
	const chatGroup = ChatGroup({companyName, chatUrl: generateMicrosoftTeamsUrl()}); // eslint-disable-line new-cap
...
const isFull = chatGroup => {
	...
	if (chatGroup.members.length >= MIN_GROUP_SIZE && chatGroup.lastMemberAddedDate + MAX_TIME_TO_WAIT_FOR_REACHING_MAX_SIZE < Date.now()) {
